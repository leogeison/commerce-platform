...
**Desde o dia 1 (sem nenhum pré-requisito real):** `UXF-001` ∥ `UXF-008` ∥ `UXF-010` ∥ `UXF-011` ∥ `UXF-012` ∥ `UXF-013` ∥ `UXE-001` ∥ `UXQ-001` (esta última só efetivamente útil depois do primeiro fluxo real existir, mas seu *scaffold* não tem pré-requisito técnico). `UXF-001A` não entra nesta lista por ter pré-requisito real (`UXF-001`), mas fica disponível assim que `UXF-001` fechar — em paralelo a `UXF-002` em diante (mesma lógica de `UXA-005A` depois de `UXA-005`, ver abaixo).

**Dentro de UXF:** depois de `UXF-001`, `UXF-001A` corre em paralelo a `UXF-002` em diante (mesmo pré-requisito único, `UXF-001`) — achado pós-fechamento de `UXF-001`/`UXF-009`, não reabre o gate.
...
**Dentro de UXW:** `UXW-001` (agora dependente também de `UXF-001A`, além de `UXF-009`) ∥ `UXW-002` (permanece dependente só de `UXF-009`); depois de `UXW-006`, `UXW-007` ∥ `UXW-008` ∥ `UXW-009`; depois de `UXW-009`, `UXW-010` ∥ `UXW-011`; `UXW-012` não depende de `UXW-009/010/011` e pode rodar assim que `UXW-007`+`UXW-008` estiverem prontos; **`UXW-014` não depende de `UXW-010/011/012/013`** e fica disponível assim que `UXW-007/008/009` terminarem — pode ser implementada bem antes de `UXW-013`, mesmo estando listada depois na numeração.
...
7. `UXW-001` a `UXW-006` podem começar assim que `UXF-010` estiver pronto (`UXW-001` também depende de `UXF-001A`, nesta revisão), sem esperar a trilha Admin. Dentro de `UXW-007…016`, aproveitar que `UXW-012` e `UXW-014` não dependem do ramo `UXW-010→011→013` — podem ser adiantadas. `UXW-011` continua sendo o ponto de espera real (depende de `UXE-018`).
