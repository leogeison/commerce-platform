...
**Dependências posteriores:** UXF-002, UXF-005, UXF-001A, todo componente visual do ciclo.

### UXF-001A — Carregamento real de Geist Sans e Source Serif 4
**Objetivo:** fechar a lacuna de carregamento real das fontes declaradas como tokens pela UXF-001 — hoje `--font-family-sans`/`--font-family-serif` resolvem apenas para a pilha de fallback de sistema, sem nenhum arquivo de fonte real carregado em nenhum dos dois apps.
**Contexto/decisão relacionada:** achado de auditoria pós-fechamento da UXF-001/UXF-009 — o próprio comentário de `packages/ui/tokens/typography.css` já registrava esta lacuna ("NENHUM carregamento real de fonte... acontece nesta tarefa"), mas nenhuma tarefa subsequente do backlog original assumiu essa responsabilidade explicitamente. Mesma disciplina já usada em UXA-005A: achado pós-fechamento de gate vira tarefa nova, sem reabrir a tarefa/gate original.
**Pré-requisitos:** UXF-001.
**Escopo incluído:** investigação, dentro desta própria tarefa, do binding correto entre `next/font` (mecanismo recomendado no Next.js atual, já dependência dos dois apps) e os tokens CSS já existentes, de forma que cada app gere sua variável de fonte exatamente sob o nome de custom property já declarado em `packages/ui/tokens/typography.css` (`--font-family-sans`/`--font-family-serif`), sem duplicar nem renomear tokens; carregamento real de Geist Sans nos dois apps (`apps/admin` e `apps/fastcompre`); carregamento real de Source Serif 4 somente em `apps/fastcompre` (nunca usada no Admin, conforme já documentado no token); preservação da pilha de fallback já declarada nos tokens para qualquer cenário de falha de carregamento.
**Fora de escopo:** qualquer alteração aos nomes ou à estrutura dos tokens em `packages/ui/tokens/typography.css`/`tailwind-theme.css`; qualquer responsabilidade de runtime em `packages/ui` — o pacote permanece exclusivamente tokens/primitives/foundations genéricas, sem nenhuma dependência de `next/font` ou lógica de carregamento (`next/font` é otimização do compilador do próprio Next.js, só invocável dentro da árvore de cada app); qualquer alteração ao budget de performance da UXF-014.
**Arquivos/áreas:** `apps/admin/src/app/layout.tsx`, `apps/fastcompre/src/app/layout.tsx`. `packages/ui` não é tocado.
**Critérios de aceite:** Geist Sans real aplicado via `--font-family-sans` nos dois apps; Source Serif 4 real aplicado via `--font-family-serif` só no FastCompre; nenhum token renomeado ou movido; fallback já declarado nos tokens permanece funcional; `pnpm build` funciona nos dois apps, local e no pipeline real de CI (incluindo o acesso de rede necessário ao `next/font` para buscar os arquivos de fonte em build-time); nenhum request externo de fonte feito em runtime pelo navegador do visitante (fontes self-hosted pelo próprio `next/font` no build); CLS medido nas 3 rotas públicas do FastCompre permanece dentro do budget da UXF-014 (Camada A, ≤ 0.10) — budget em si não é alterado por esta tarefa.
**Testes esperados:** verificação de build de produção real dos dois apps (mesmo critério já usado em UXF-013); medição de CLS nas 3 rotas do FastCompre contra os critérios PASS/FAIL/UNKNOWN já definidos na UXF-014; suítes existentes dos dois apps continuam verdes sem alteração de comportamento não relacionado.
**Performance:** critério de aceite central desta tarefa quanto a CLS — medir contra o budget já existente da UXF-014, nunca redefini-lo.
**Riscos:** `next/font` busca os arquivos de fonte durante o build, o que exige acesso de rede do pipeline de CI ao provedor de fonte — verificar explicitamente nesta tarefa, já que a rede do CI segue allowlist restrito.
**Dependências posteriores:** UXW-001.
...
**Dependências posteriores:** UXF-010A.

### UXF-010A — Revalidação global de Categorias
**Objetivo:** garantir que toda criação, rename, arquivamento ou desarquivamento de Categoria concluído com sucesso dispare exatamente uma tentativa de revalidação sitewide (`revalidatePath('/', 'layout')`) no FastCompre, para que a navegação pública introduzida pela UXW-003 reflita o estado real das Categorias não-arquivadas do Site, independentemente de haver Article `PUBLISHED` vinculado.
**Contexto/decisão relacionada:** achado da investigação da UXW-003 — hoje a revalidação de Categoria só ocorre no caminho de rename (`UpdateCategoryAndRevalidateUseCase`), condicionada à descoberta de Artigos `PUBLISHED` afetados (`RevalidateAffectedArticlesUseCase`), podendo resultar em zero chamadas; criação, arquivamento e desarquivamento de Categoria não disparam revalidação em nenhuma condição hoje. A partir da UXW-003, Categoria passa a alimentar uma superfície pública global (o menu), e não mais só os Artigos que a referenciam — a dependência de descoberta de Artigo deixa de ser suficiente. Decisão aprovada: manter a estratégia de invalidação já existente, orientada a path/layout (`revalidatePath('/', 'layout')`), sem TTL e sem cache tags — nenhuma infraestrutura de cache nova. A investigação confirmou que o mecanismo do Next.js (Data Cache/Full Route Cache) já é suficiente; a lacuna está inteiramente no lado da API, que hoje não aciona a revalidação para Categoria de forma incondicional.
**Pré-requisitos:** UXF-010.
**Escopo incluído:** `packages/contracts/src/internal/revalidate-request.ts` — tornar `articleSlug` opcional no contrato (mudança aditiva; todo consumidor existente já envia um Artigo real); introdução, na camada `application` da API, de uma responsabilidade de composição entre a mutação de Categoria (criar/arquivar/desarquivar/renomear) e a revalidação pública, garantindo exatamente uma tentativa de `revalidatePath('/', 'layout')` por mutação concluída com sucesso, sem depender de descoberta de Artigo; `CategoriesController.create()`/`.archive()`/`.unarchive()` passam a delegar a essa composição em vez de chamar os use cases de catálogo diretamente; o caminho de rename (`UpdateCategoryAndRevalidateUseCase`) passa a usar essa mesma composição em vez de `RevalidateAffectedArticlesUseCase.revalidateForCategory()`; `Architecture.md` §21 atualizado para registrar a regra. A composição deve viver na camada de aplicação (orquestração), nunca no domínio, no repositório ou no controller — os nomes exatos das classes/métodos serão fechados no desenho técnico desta tarefa, antes da implementação.
**Fora de escopo:** revalidação de Autor/Produto/Oferta (comportamento existente, via `RevalidateAffectedArticlesUseCase`, permanece inalterado); qualquer TTL ou cache tag; a implementação da navegação da UXW-003 em si; qualquer alteração à máquina de estados do Artigo ou às regras de publicação/autorização já existentes; qualquer alteração à política de falha/best-effort já estabelecida (`Architecture.md` §21, "Natureza síncrona e tratamento de falha") — falha na revalidação continua apenas logada, sem reverter a mutação já persistida, sem fila/retry/outbox.
**Arquivos/áreas:** `packages/contracts/src/internal/revalidate-request.ts`; `apps/api/src/modules/application/application/` (novo componente de orquestração e ajuste de `update-category-and-revalidate.use-case.ts`); `apps/api/src/modules/catalog/presentation/categories.controller.ts`; `Architecture.md` (§21).
**Critérios de aceite:** Cada criação, rename, arquivamento ou desarquivamento de Categoria concluído com sucesso realiza exatamente uma tentativa de revalidação sitewide, independentemente da quantidade de Articles PUBLISHED relacionados. `revalidatePath('/', 'layout')` é acionado nos quatro caminhos (criar/renomear/arquivar/desarquivar); nenhuma chamada redundante é introduzida; comportamento de revalidação de Artigo, Autor, Produto e Oferta permanece idêntico ao atual; falha na revalidação continua não revertendo a mutação já persistida (política existente preservada).
**Testes esperados:** unit (novo componente de orquestração: exatamente uma chamada por mutação, independente de Artigos afetados; propagação correta de falha para log sem reversão da mutação); e2e ou de integração cobrindo os quatro caminhos de mutação de Categoria acionando o endpoint interno de revalidação.
**Riscos:** nenhum além do já mapeado — mudança aditiva e localizada à orquestração de Categoria; não afeta Artigo/Autor/Produto/Oferta.
...
**Pré-requisitos:** UXF-009, UXF-001A.
...
**Contexto/decisão relacionada:** Etapa C — menu de Categorias aprovado como capacidade mandatória do header, dependente do pré-requisito técnico; Etapa D/UXF-010 — endpoint implementado; `UXF-010A` — pré-requisito corretivo que garante revalidação sitewide de Categoria independentemente de vínculo com Artigo `PUBLISHED`, nesta revisão.
**Pré-requisitos:** UXW-001, UXF-010A.
...
**Riscos:** nenhum.
**Dependências posteriores:** UXW-005A.

### UXW-005A — JSON-LD estrutural de Article/Product
**Objetivo:** corrigir dívida estrutural identificada na investigação da UXW-006 — implementar os dados estruturados (JSON-LD) de `Article`/`Product` que `Architecture.md` §29/§33 determinam como decisão já aprovada, ausente desde o MVP (WEB-005).
**Contexto/decisão relacionada:** `Architecture.md` §29/§33. Decisão do Product Owner: JSON-LD é requisito normativo já devido, corrige lacuna do MVP, não antecipa UXW-007+. `Offer.seller` fica fora de escopo — os enums atuais de `marketplace` são provisórios (a serem revisados antes de produção); nenhum mapeamento de nome comercial é criado nesta tarefa, nem qualquer refatoração do enum/modelo `marketplace`.
**Pré-requisitos:** UXW-005.
**Escopo incluído:** `@graph` JSON-LD (`Article` + um `Product` por item de `article.products[]`), com `Article.mentions` referenciando os Produtos via `@id` (sem duplicação); extração de `affiliateRedirectHref()` (hoje interna a `page.tsx`) para um módulo dedicado, única fonte de verdade reutilizada tanto pelo link visível quanto por `Offer.url` no JSON-LD, sem alterar sua lógica/assinatura; Produto sem Ofertas omite a propriedade `offers` inteira (nunca `offers: []`); derivado inteiramente de `PublicArticle`/`PublicArticleAuthor`/`PublicArticleProduct`/`PublicOffer` — nenhum dado novo, nenhuma nova chamada à API pública.
**Fora de escopo:** JSON-LD em Home/Categoria; `Article.dateModified`; `Article.publisher.logo`; `Offer.seller`/mapeamento de nome de marketplace/refatoração do enum `marketplace`/entidade dinâmica de provider; `<link rel="canonical">`/`alternates.canonical`; qualquer mecanismo novo de tolerância/fallback para dado fora do shape já validado pelos contratos públicos.
**Arquivos/áreas:** `apps/fastcompre/src/app/[categorySlug]/[articleSlug]/affiliate-redirect-href.ts` (novo), `affiliate-redirect-href.spec.ts` (novo), `article-json-ld.ts` (novo), `article-json-ld.spec.ts(x)` (novo), `page.tsx` (modificado — extração + renderização do script). `page.spec.tsx` (um dos 10 specs históricos) não é modificado.
**Critérios de aceite:** `<script type="application/ld+json">` presente na página de Artigo com `@graph` (`Article` + um `Product` por item real, `mentions` referenciando-os por `@id`, `offers` omitida quando vazia); `Offer.url` idêntico ao link visível de afiliado, produzido pela mesma função compartilhada (nenhuma duplicação de lógica de URL); nenhuma propriedade inventada sem dado real correspondente; serialização segura contra quebra de tag/XSS; nenhum campo persistido em Prisma; nenhuma mudança de contrato/API; `generateMetadata`, redirect canônico, sitemap e robots inalterados; os 10 specs históricos e o restante da suíte continuam intactos.
**Testes esperados:** spec da função extraída `affiliateRedirectHref` (comportamento idêntico ao atual); spec dedicado da derivação do JSON-LD (dado nulo/presente, 0/N Produtos/Ofertas, `offers` omitida quando vazia, `@id`/`mentions` corretos); teste de round-trip de escape (`</script>`/`<img onerror>` em campo real não quebra a página e sobrevive ao `JSON.parse`); teste de integração confirmando o script no HTML real da página e que `Offer.url` bate exatamente com o `href` visível.
**Riscos:** validade estrutural do JSON-LD não é verificada por nenhum gate automatizado existente além do spec dedicado desta tarefa (nenhuma ferramenta externa de validação de rich results faz parte do CI); risco de dessincronização entre o JSON-LD e o conteúdo visível se uma tarefa futura alterar um sem o outro — mitigado por ambos derivarem do mesmo objeto `article` e da mesma função `affiliateRedirectHref`, mas não é uma garantia permanente; risco de serialização insegura (quebra de `<script>`) se o escape de `<` for removido/esquecido em alteração futura — mitigado pelo teste de round-trip dedicado, que deve permanecer como gate.
**Dependências posteriores:** UXW-006.

### UXW-006 — Gate de fechamento: shell público
**Objetivo:** marco de fechamento da navegação pública nova.
**Contexto/decisão relacionada:** fecha E9. A medição intermediária de performance do próprio gate encontrou uma regressão real de LCP em Home e Categoria (as 3 rotas fora do teto de regressão da Camada B da UXF-014; Home e Artigo também fora do teto absoluto de 2500 ms) — investigação por evidência (relatório Lighthouse: `resourceLoadDelay`/ausência de `fetchpriority=high should be applied`) isolou a causa em Home/Categoria na primeira thumbnail da listagem de artigos (`img.aspect-video`), servida com `loading="lazy"` e sem `fetchPriority`, sem nenhuma prioridade de carregamento acima do fold. UXW-006 passa a incluir a correção mínima desse ponto antes da revalidação final do gate — decisão explícita de manter a correção dentro da própria UXW-006, não como tarefa corretiva separada. Aplicada essa correção, a revalidação mostrou ganho real porém insuficiente — as 3 rotas seguiram fora do budget, incluindo o Artigo (cujo elemento de LCP é texto, não imagem). Nova investigação por evidência (relatórios Lighthouse) isolou um recurso CSS compartilhado e render-blocking, amarrado ao `layout.tsx` raiz (portanto presente nas 3 rotas): o `globals.css` compilado cresceu desde o baseline UXF-013 por conta das classes Tailwind reais de `SiteHeader`/`SiteFooter`/`CategoryNav` (UXW-001/002/003/004) e das variáveis de fonte (UXF-001A) — crescimento legítimo, não um defeito —, e `category-nav.module.css` é um segundo recurso CSS novo também amarrado ao shell raiz; nenhum dos dois existia no baseline. Investigação subsequente no código/types do `next@16.2.12` instalado confirmou que `experimental.inlineCss` (App Router, produção, sem dependência adicional) era a opção suportada para eliminar o round-trip de rede desse CSS bloqueante, enquanto `experimental.optimizeCss` seria um no-op neste projeto (só se aplica ao Pages Router nesta versão) e `experimental.cssChunking` já está no padrão ideal (`true`, ativo desde sempre). UXW-006 passou a incluir o teste isolado de `experimental.inlineCss` antes da revalidação final do gate, com permanência condicionada a evidência empírica da medição normativa — decisão explícita de manter o teste dentro da própria UXW-006, não como tarefa corretiva separada. **Resultado do teste (medição normativa local, 5 execuções por rota, fix de imagem já aplicado em ambos os cenários): sem `inlineCss` — Home 2560,02 ms / Categoria 2480,98 ms / Artigo 2465,14 ms; com `inlineCss` — Home 2715,47 ms / Categoria 2633,58 ms / Artigo 2625,67 ms — regressão uniforme de ~153–161 ms nas 3 rotas.** `experimental.inlineCss` foi reprovada empiricamente e revertida por completo de `apps/fastcompre/next.config.ts`, que volta ao estado anterior ao experimento. A causa raiz do CSS render-blocking compartilhado permanece identificada (ver acima) mas ainda sem correção adotada — a UXW-006 segue aberta nesse ponto, aguardando nova investigação/decisão.
**Pré-requisitos:** UXW-005A.
**Escopo incluído:** verificação de que SSR/SEO das 3 rotas existentes não regrediu (título, meta description, JSON-LD, URL canônica todos preservados); correção mínima da regressão de LCP encontrada em Home e Categoria — a primeira imagem da listagem de artigos com `coverImageUrl` (nunca `index === 0` cru, já que o campo é opcional) passa a `loading="eager"` + `fetchPriority="high"`; todas as demais imagens continuam `loading="lazy"`, sem prioridade; sem imagem nenhuma na lista, comportamento inalterado.
**Fora de escopo:** conteúdo interno das páginas (UXW-007+); qualquer correção no Artigo (variabilidade específica do Artigo, se houver, tratada separadamente); fontes (UXF-001A), Header/`Suspense` (UXW-001), Footer (UXW-002), `CategoryNav`/drawer (UXW-003/UXW-004), JSON-LD (UXW-005A) — descartados como causa por evidência (ver investigação desta tarefa), não tocados; extração de componente compartilhado entre Home/Categoria (a duplicação de markup já existia antes desta correção); adoção de `next/image`; qualquer mudança de budget; `experimental.optimizeCss`/Critters/Beasties (não suportado para App Router nesta versão do Next, não instalado); `experimental.cssChunking` (mantido no padrão, não alterado); `experimental.inlineCss` (testada isoladamente nesta tarefa — ver Contexto/decisão relacionada — e revertida por reprovação empírica; não faz parte do escopo entregue).
**Arquivos/áreas:** `apps/fastcompre/src/app/page.tsx` (modificado — prioridade de carregamento da primeira imagem), `apps/fastcompre/src/app/page.spec.tsx` (modificado — testes da correção), `apps/fastcompre/src/app/[categorySlug]/page.tsx` (modificado, mesma correção), `apps/fastcompre/src/app/[categorySlug]/page.spec.tsx` (modificado — testes da correção). Nenhum outro arquivo. (`apps/fastcompre/next.config.ts` foi temporariamente modificado para o teste de `experimental.inlineCss` e revertido ao estado original após a reprovação empírica — não consta do escopo entregue.)
**Critérios de aceite:** pipeline completo verde; SEO estrutural preservado byte a byte onde aplicável; a regressão de LCP de Home e Categoria corrigida pela correção de imagem e revalidada pela medição normativa (`scripts/measure-performance-baseline.mjs`, mesmos slugs, 5 execuções por rota); Artigo segue fora do budget mesmo após a correção de imagem — permanece pendência aberta desta tarefa, não resolvida por `inlineCss` (reprovada).
**Testes esperados:** suíte completa do shell; testes dedicados provando que só a primeira imagem elegível (primeiro artigo da lista com `coverImageUrl`) recebe `loading="eager"`/`fetchPriority="high"`, inclusive quando o primeiro artigo da lista não tem imagem, e que o comportamento é inalterado quando nenhum artigo tem imagem.
**Performance:** medição intermediária contra o budget de UXF-014 (não a medição final, que é UXQ-007); a primeira medição intermediária (Home/Categoria/Artigo fora do budget) motivou a correção de imagem registrada acima; medição seguinte (ainda fora do budget nas 3 rotas) motivou o teste de `experimental.inlineCss`; a medição normativa pós-`inlineCss` (5 execuções por rota, local) reprovou a flag (regressão uniforme de ~153–161 ms) — revertida. Home/Categoria/Artigo permanecem a revalidar com uma correção diferente para o CSS render-blocking compartilhado antes do fechamento do gate.
**Riscos:** causa raiz do CSS render-blocking compartilhado (crescimento legítimo de `globals.css`/`category-nav.module.css` amarrado ao shell raiz) segue sem mitigação adotada — `inlineCss` foi a primeira tentativa e não se sustentou empiricamente; Artigo permanece fora do budget até nova correção ser investigada e validada.
...
**Riscos:** nenhum.
**Dependências posteriores:** UXW-010A.

### UXW-010A — Remover disclosure duplicado da página de Artigo
**Objetivo:** eliminar a duplicidade de divulgação de afiliação na página de Artigo, tornando o `SiteFooter` (UXW-002) a única fonte canônica de disclosure sitewide, conforme nova decisão do Product Owner — a página de Artigo deixa de repetir um parágrafo de divulgação próprio abaixo do título.
**Contexto/decisão relacionada:** achado da validação visual da `UXW-010` — a página de Artigo (`UXW-009`) exibia um parágrafo de divulgação de afiliação próprio, duplicando o `SiteFooter` (`UXW-002`), já sempre visível em todas as rotas públicas via `layout.tsx`. `site-footer.tsx` documentava essa duplicidade como decisão consciente e já aprovada do Product Owner à época da `UXW-002`. Nova decisão do Product Owner, registrada nesta tarefa, substitui essa decisão anterior: haverá uma única divulgação de afiliação, no `SiteFooter`, sempre visível em todas as rotas (inclusive Artigo); a página de Artigo não repete um segundo disclosure. `Architecture.md` §7 (documento normativo do Project, não versionado no repositório) é ajustado nesta mesma tarefa para não presumir mais divulgação por artigo; `Architecture.md` §33 foi avaliado e não exige alteração; `docs/UXW-visual-direction-decision.md` foi revisado e não contém nenhum trecho que determine a posição da divulgação na página de Artigo — nenhuma alteração necessária nesse documento.
**Pré-requisitos:** UXW-010.
**Escopo incluído:** remoção do parágrafo de divulgação específico da página de Artigo (`page.tsx`, o `<p>` entre `<h1>` e `<time>`); ajuste do único teste que dependia desse parágrafo (`page.spec.tsx` — remoção da asserção `expect(html).toContain('links de afiliados')` e renomeação do teste correspondente); atualização do comentário de `site-footer.tsx` para refletir a nova decisão; atualização de `Architecture.md` §7 (documento do Project, atualizado separadamente do repositório).
**Fora de escopo:** qualquer alteração ao texto de divulgação em si; `SiteFooter` (UXW-002, já implementado — só seu comentário é atualizado, sem mudança de conteúdo/comportamento); `site-footer.spec.tsx` e `shell.integration.spec.tsx` (preservados sem alteração); metadata, JSON-LD (`article-json-ld.ts`), canonical, CTA/tracking de afiliado, byline (UXW-010), `bodyMdx`, bloco Produto/Oferta e o Editorial Serialization Contract; qualquer relação de dependência com `UXW-011` — não introduzida por esta tarefa.
**Arquivos/áreas:** `apps/fastcompre/src/app/[categorySlug]/[articleSlug]/page.tsx`, `apps/fastcompre/src/app/[categorySlug]/[articleSlug]/page.spec.tsx`, `apps/fastcompre/src/app/site-footer.tsx` (só o comentário), `Architecture.md` §7 (Project, fora do repositório).
**Critérios de aceite:** a página de Artigo não renderiza mais nenhum parágrafo de divulgação de afiliação próprio; o `SiteFooter` continua exibindo a divulgação exatamente como hoje, em todas as rotas incluindo Artigo; nenhuma outra seção da página de Artigo é alterada; suíte de `page.spec.tsx` reflete a remoção sem perda de cobertura das demais asserções; `site-footer.spec.tsx` e `shell.integration.spec.tsx` permanecem verdes sem modificação; `Architecture.md` §7 não presume mais divulgação por artigo.
**Testes esperados:** suíte completa de `page.spec.tsx` (com o teste ajustado/renomeado) verde; `site-footer.spec.tsx` e `shell.integration.spec.tsx` verdes sem alteração; typecheck/lint/build sem regressão.
**Acessibilidade/responsividade:** nenhuma mudança de mecanismo — remove um `<p>` estático; divulgação continua acessível via `SiteFooter`, landmark `<footer>`, já validado na UXW-002.
**Riscos:** menor proeminência contextual da divulgação em artigos longos, pois o disclosure único permanece no footer; esta tarefa registra a decisão de produto e não redefine requisitos externos de programas de afiliados ou obrigações legais.
...
**Pré-requisitos:** UXW-007, UXW-008, UXW-009, UXW-010A.
...
UXF (Foundation)
  UXF-001 → UXF-002
  UXF-001 → UXF-001A                          (∥ ao ramo 002 em diante — mesmo pré-requisito único, UXF-001; achado pós-fechamento de UXF-001/UXF-009, não reabre o gate UXF-009)
  ...
  UXF-010 → UXF-010A                          (corretivo — ver UXF-010A; achado da investigação da UXW-003, registrado nesta revisão só para fins normativos, não reabre UXF-009)
...
UXW (FastCompre)
  ...
  UXF-009 → UXW-002                            (∥ a UXW-001 — depende só de UXF-009, não de UXF-001A)
  {UXW-001, UXF-010A} → UXW-003              (UXF-010A no lugar do UXF-010 direto — ver UXF-010A; cadeia preservada: UXF-010 → UXF-010A → UXW-003)
  {UXW-003, UXF-008} → UXW-004
  {UXW-001, UXW-002, UXW-003, UXW-004} → UXW-005 → UXW-005A → UXW-006 (gate E9) (UXW-005A inserida nesta revisão — ver UXW-005A; corrige dívida de JSON-LD achada na investigação da UXW-006, que agora depende de UXW-005A no lugar de UXW-005 diretamente)
  UXW-006 → UXW-007 ∥ UXW-008 ∥ UXW-009        (três dependem só de UXW-006)
  {UXW-009, UXF-011} → UXW-010                 (∥ a UXW-011, ambas partem de UXW-009)
  UXW-010 → UXW-010A                            (corretivo — remove o disclosure duplicado do Artigo; nenhuma dependência com UXW-011)
  {UXW-009, UXE-018} → UXW-011
  {UXW-007, UXW-008} → UXW-012                 (não depende de UXW-009/010/011)
  {UXW-007, UXW-008, UXW-009, UXW-010A} → UXW-013   (UXW-010A no lugar de UXW-010 — corretivo desta revisão; cadeia preservada: UXW-010 → UXW-010A → UXW-013)
...
**Caminho C — FastCompre:** `(UXF-009 + UXF-001A → UXW-001) + (UXF-010 → UXF-010A) → UXW-003 → UXW-004 → UXW-005 → UXW-005A → UXW-006 (gate) → {UXW-007∥UXW-008∥UXW-009} → ... → UXW-016`, com `UXW-014` disponível cedo (logo após `UXW-007/008/009`, sem esperar `UXW-010…013`) e `UXW-011` bloqueada especificamente por `UXE-018` — ou seja, o Caminho C só fecha de fato depois que o ramo público de `UXE` (não o ramo do editor) estiver pronto. Por correção de dependência, `UXW-011` também é pré-requisito de `UXE-021` (Caminho B) — o acoplamento entre os dois caminhos passa a ser nos dois sentidos, não só de C para B. `UXF-001A` (nesta revisão) entra explicitamente como pré-requisito de `UXW-001`, no início deste caminho, sem alongá-lo: `UXF-001A` depende só de `UXF-001`, então corre em paralelo a `UXF-009`/`UXF-010`/`UXF-002…008`, e não é o gate mais lento do conjunto. `UXF-010A` (nesta revisão) substitui `UXF-010` como pré-requisito direto de `UXW-003` neste caminho, sem alongá-lo: `UXF-010A` depende só de `UXF-010`, então corre em paralelo ao restante de `UXF-001…009`/`UXF-011`/`UXF-012`, e não é o gate mais lento do conjunto — a cadeia transitiva `UXF-010 → UXF-010A → UXW-003` preserva integralmente a dependência original. `UXW-005A` (nesta revisão) entra entre `UXW-005` e `UXW-006`, sem alongar o caminho crítico de forma inesperada — é o achado da própria investigação de `UXW-006`, que passa a depender de `UXW-005A` em vez de `UXW-005` diretamente: corrige uma dívida estrutural do MVP (JSON-LD ausente) antes de `UXW-006` fechar, não antecipa nenhuma tarefa de `UXW-007` em diante. `UXW-010A` (nesta revisão) é inserida entre `UXW-010` e `UXW-013`, sem alongar o caminho crítico: depende só de `UXW-010` e não introduz nenhuma dependência com `UXW-011`, ramo paralelo; `UXW-013` passa a depender de `UXW-010A` no lugar de `UXW-010` diretamente (cadeia transitiva preservada: `UXW-010 → UXW-010A → UXW-013`).
...
**Desde o dia 1 (sem nenhum pré-requisito real):** `UXF-001` ∥ `UXF-008` ∥ `UXF-010` ∥ `UXF-011` ∥ `UXF-012` ∥ `UXF-013` ∥ `UXE-001` ∥ `UXQ-001` (esta última só efetivamente útil depois do primeiro fluxo real existir, mas seu *scaffold* não tem pré-requisito técnico). `UXF-001A` não entra nesta lista por ter pré-requisito real (`UXF-001`), mas fica disponível assim que `UXF-001` fechar — em paralelo a `UXF-002` em diante (mesma lógica de `UXA-005A` depois de `UXA-005`, ver abaixo). Pela mesma razão, `UXF-010A` também não entra nesta lista — depende de `UXF-010` — mas fica disponível assim que `UXF-010` fechar, em paralelo ao restante de `UXF-001…009`/`UXF-011`/`UXF-012`.

**Dentro de UXF:** depois de `UXF-001`, `UXF-001A` corre em paralelo a `UXF-002` em diante (mesmo pré-requisito único, `UXF-001`) — achado pós-fechamento de `UXF-001`/`UXF-009`, não reabre o gate. Depois de `UXF-010`, `UXF-010A` corre em paralelo ao restante do bloco (`UXF-011`/`UXF-012`/`UXF-013→014`, mesmo escopo temporal) — achado da investigação da `UXW-003`, corretivo de revalidação de Categoria, não reabre nenhum gate já fechado.
...
**Dentro de UXW:** `UXW-001` (agora dependente também de `UXF-001A`, além de `UXF-009`) ∥ `UXW-002` (permanece dependente só de `UXF-009`); `UXW-003` passa a depender de `UXF-010A` no lugar de `UXF-010` (cadeia transitiva preservada: `UXF-010 → UXF-010A → UXW-003`); depois de `UXW-005`, `UXW-005A` corre em sequência (não paralela — é dívida corretiva bloqueante de `UXW-006`, não uma tarefa independente); depois de `UXW-006`, `UXW-007` ∥ `UXW-008` ∥ `UXW-009`; depois de `UXW-009`, `UXW-010` ∥ `UXW-011`; depois de `UXW-010`, `UXW-010A` corre em sequência (corretivo, não paralelo) antes de `UXW-013`, sem depender de/gerar dependência com `UXW-011`; `UXW-012` não depende de `UXW-009/010/011` e pode rodar assim que `UXW-007`+`UXW-008` estiverem prontos; **`UXW-014` não depende de `UXW-010/011/012/013`** e fica disponível assim que `UXW-007/008/009` terminarem — pode ser implementada bem antes de `UXW-013`, mesmo estando listada depois na numeração.
...
7. `UXW-001` e `UXW-002` podem começar assim que `UXF-010` estiver pronto (`UXW-001` também depende de `UXF-001A`, nesta revisão), sem esperar a trilha Admin. `UXW-003` (e, por dependência transitiva, `UXW-004`, `UXW-005`, `UXW-005A` e `UXW-006`) passa a depender também de `UXF-010A` — o pré-requisito corretivo registrado nesta revisão — e só pode começar depois de `UXF-010A` fechar, ainda que `UXF-010` já esteja pronto. `UXW-005A` (nesta revisão) é um passo corretivo entre `UXW-005` e `UXW-006` — implementa o JSON-LD estrutural de `Article`/`Product` que a investigação da própria `UXW-006` encontrou ausente; `UXW-006` fica bloqueada até `UXW-005A` fechar. Dentro de `UXW-007…016`, aproveitar que `UXW-012` e `UXW-014` não dependem do ramo `UXW-010→011→013` — podem ser adiantadas. `UXW-011` continua sendo o ponto de espera real (depende de `UXE-018`).
