...
describe('CategoryList', () => {
  ...
  // --- UXA-001: LoadingState/ErrorState/EmptyState compartilhados ---
  ...
  it('UXA-001: estado de loading não usa role="alert" e não trava foco (sem violação de acessibilidade)', async () => {
    ...
    expect(await screen.findByRole('link', { name: 'Nova Categoria' })).toBeInTheDocument();

    expect(await axe(container)).toHaveNoViolations();
  });

  it('UXA-001: estado de erro é anunciado via role="alert" (sem violação de acessibilidade)', async () => {
    global.fetch = jest.fn<typeof fetch>().mockResolvedValue(jsonResponse(500, { unexpected: 'shape' }));
    const { container } = renderList();
  ...
  it('UXA-001: estado vazio sem violação de acessibilidade', async () => {
    global.fetch = jest
      .fn<typeof fetch>()
      .mockResolvedValue(jsonResponse(200, { items: [], page: 1, pageSize: 20, total: 0, totalPages: 0 }));
    const { container } = renderList();
