...
const USER_EMAIL = 'edt016-user@test.com';
const NONEXISTENT_ID = '00000000-0000-0000-0000-000000000000';

/**
 * `POST /admin/sites/:siteSlug/articles/:id/restore-to-draft` (e2e,
 * EDT-016). Exige Postgres real (mesmo requisito dos demais e2e do
 * projeto).
 */
describe('POST /admin/sites/:siteSlug/articles/:id/restore-to-draft (e2e)', () => {
  ...
  afterEach(async () => {
    ...
    await prisma.article.deleteMany({
      where: { site: { slug: { startsWith: 'edt016-' } } },
    });
    await prisma.siteUser.deleteMany({
      where: { site: { slug: { startsWith: 'edt016-' } } },
    });
    await prisma.site.deleteMany({ where: { slug: { startsWith: 'edt016-' } } });
    if (user?.id) {
  ...
  function restoreRequest(site: Site, articleId: string) {
  ...
  it('ARCHIVED → DRAFT: 200, corpo válido contra articleAdminSchema', async () => {
    await setRole(siteA, Role.OWNER);
    const article = await createArticle(siteA, 'artigo-restore-sucesso', ArticleStatus.ARCHIVED);

    const response = await restoreRequest(siteA, article.id);
  ...
  it.each([ArticleStatus.DRAFT, ArticleStatus.PENDING_REVIEW, ArticleStatus.PUBLISHED])(
    'status de origem inválido (%s): 409, status não muda',
    async (status) => {
      await setRole(siteA, Role.OWNER);
      const article = await createArticle(siteA, `artigo-restore-invalido-${status.toLowerCase()}`, status);
  ...
  it('articleId inexistente no próprio Site: 404', async () => {
    ...
    const response = await restoreRequest(siteA, NONEXISTENT_ID);
  ...
  it('articleId de um Artigo real de outro Site: 404 (isolamento)', async () => {
    ...
    const response = await restoreRequest(siteA, articleFromSiteB.id);
  ...
  it('Role insuficiente (EDITOR, abaixo de OWNER): 403', async () => {
    await setRole(siteA, Role.EDITOR);
    const article = await createArticle(siteA, 'artigo-restore-role-insuficiente');
  ...
  it('Origin inválida: 403', async () => {
    await setRole(siteA, Role.OWNER);
    const article = await createArticle(siteA, 'artigo-restore-origem-invalida');

    const response = await request(app!.getHttpServer())
      .post(`/admin/sites/${siteA.slug}/articles/${article.id}/restore-to-draft`)
      .set('Cookie', cookieHeader())
  ...
  it('sem autenticação (sem cookie): 401', async () => {
    await setRole(siteA, Role.OWNER);
    const article = await createArticle(siteA, 'artigo-restore-sem-sessao');
  ...
  it('articleId com formato inválido (não-UUID): 422', async () => {
    ...
    const response = await restoreRequest(siteA, 'nao-e-um-uuid');
