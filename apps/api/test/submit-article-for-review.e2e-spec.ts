...
const USER_EMAIL = 'edt012-user@test.com';
const NONEXISTENT_ID = '00000000-0000-0000-0000-000000000000';

/**
 * `POST /admin/sites/:siteSlug/articles/:id/submit-for-review` (e2e,
 * EDT-012). Exige Postgres real (mesmo requisito dos demais e2e do
 * projeto).
 */
describe('POST /admin/sites/:siteSlug/articles/:id/submit-for-review (e2e)', () => {
  ...
  afterEach(async () => {
    ...
    await prisma.article.deleteMany({
      where: { site: { slug: { startsWith: 'edt012-' } } },
    });
    await prisma.siteUser.deleteMany({
      where: { site: { slug: { startsWith: 'edt012-' } } },
    });
    await prisma.site.deleteMany({ where: { slug: { startsWith: 'edt012-' } } });
    if (user?.id) {
  ...
  function submitRequest(site: Site, articleId: string) {
  ...
  it('DRAFT → PENDING_REVIEW: 200, corpo válido contra articleAdminSchema, publishedAt permanece null', async () => {
    await setRole(siteA, Role.EDITOR);
    const article = await createArticle(siteA, 'artigo-submit-sucesso', ArticleStatus.DRAFT);

    const response = await submitRequest(siteA, article.id);
  ...
  it.each([ArticleStatus.PENDING_REVIEW, ArticleStatus.PUBLISHED, ArticleStatus.ARCHIVED])(
    ...
    async (status) => {
      await setRole(siteA, Role.EDITOR);
      const article = await createArticle(siteA, `artigo-submit-invalido-${status.toLowerCase()}`, status);
  ...
  it('articleId inexistente no próprio Site: 404', async () => {
    ...
    const response = await submitRequest(siteA, NONEXISTENT_ID);
  ...
  it('articleId de um Artigo real de outro Site: 404 (isolamento)', async () => {
    ...
    const response = await submitRequest(siteA, articleFromSiteB.id);
  ...
  it('Role insuficiente (VIEWER): 403', async () => {
    ...
    const article = await createArticle(siteA, 'artigo-submit-role-insuficiente');
  ...
  it('Origin inválida: 403', async () => {
    await setRole(siteA, Role.EDITOR);
    const article = await createArticle(siteA, 'artigo-submit-origem-invalida');

    const response = await request(app!.getHttpServer())
      .post(`/admin/sites/${siteA.slug}/articles/${article.id}/submit-for-review`)
      .set('Cookie', cookieHeader())
  ...
  it('sem autenticação (sem cookie): 401', async () => {
    await setRole(siteA, Role.EDITOR);
    const article = await createArticle(siteA, 'artigo-submit-sem-sessao');
  ...
  it('articleId com formato inválido (não-UUID): 422', async () => {
    ...
    const response = await submitRequest(siteA, 'nao-e-um-uuid');
